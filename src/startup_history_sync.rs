...
#[cfg(test)]
mod tests {
    use super::StartupHistorySyncService;
    use crate::transport::message::{
        Conversation, ConversationKind, IncomingMessage, MessageContent, Participant,
    };

    #[test]
    fn keeps_only_messages_after_the_last_bot_message() {
        let messages = vec![
            test_message("1", "user"),
            test_message("2", "bot"),
            test_message("3", "user"),
        ...
        let result = StartupHistorySyncService::messages_after_last_bot(messages, "bot");

        assert_eq!(result.len(), 1);
        assert_eq!(result[0].message_id.as_deref(), Some("5"));
    }

    #[test]
    fn keeps_all_messages_when_bot_has_not_spoken_in_returned_history() {
        let messages = vec![test_message("1", "user"), test_message("2", "user")];
    ...
    fn test_message(message_id: &str, sender_id: &str) -> IncomingMessage {
        IncomingMessage {
