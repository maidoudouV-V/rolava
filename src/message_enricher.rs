...
impl MessageEnricher {
    ...
    pub async fn enrich(&self, mut message: IncomingMessage) -> IncomingMessage {
        let image_indexes: Vec<usize> = message.content.parts
            .iter()
            .enumerate()
        ...
        for image_index in image_indexes {
            ...
            message.content.text = Self::replace_next_image_placeholder(
    ...
    async fn enrich_image_part(&self, image_data: &Value) -> Result<Option<EnrichedImage>> {
        ...
        let description = self.describe_image(
        ...
        let local_path = self.save_image_file(
            &image_id,
            downloaded_image.mime_type.as_deref(),
            &downloaded_image.bytes,
        ).await?;
    ...
    async fn describe_image(&self, bytes: &[u8], mime_type: Option<&str>) -> Result<String> {
        ...
        let visual_provider = self.app_config
            .ai_providers
        ...
        for attempt in 1..=2 {
            match visual_provider
                .describe_image(&image_data_url, IMAGE_DESCRIPTION_PROMPT)
                .await
            {
                Ok(description) => return Ok(Self::sanitize_description(&description)),
                Err(err) => {
                    eprintln!("图片识别 API 请求失败，第 {} 次: {}", attempt, err);
                    last_error = Some(err);
                }
            }
        }
    ...
    fn prepare_image_for_vision(bytes: &[u8], mime_type: Option<&str>) -> Result<VisionImagePayload> {
        ...
        let decoded_image = match image::load_from_memory(bytes) {
            ...
            Err(err) if bytes.len() <= VISION_IMAGE_MAX_BYTES => {
                eprintln!("解析待压缩图片失败，图片体积未超过限制，使用原图发送视觉模型: {}", err);
    ...
    fn attach_image_info(image_data: &mut Value, image: &EnrichedImage) {
        if let Value::Object(map) = image_data {
            map.insert("image_id".to_string(), Value::String(image.image_id.clone()));
            map.insert("content_hash".to_string(), Value::String(image.content_hash.clone()));
            map.insert("local_path".to_string(), Value::String(image.local_path.clone()));
            map.insert("description".to_string(), Value::String(image.description.clone()));
        }
    }
